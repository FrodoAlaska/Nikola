...
inline nikola::GfxShaderDesc generate_instance_shader() {
  return nikola::GfxShaderDesc {
    ...
    "\n"
    "void main() {\n"
    "  gl_Position = u_projection * u_view * u_model[gl_InstanceID] * vec4(aPos, 1.0f);\n"
    "\n"
    "  vs_out.normal         = aNormal;\n"
    ...
    ""
    "void main() {\n"
    "  frag_color = texture(u_material.diffuse_map, fs_in.texture_coords) * vec4(u_material.color, u_material.transparency);\n"
    "}"
  };
}
