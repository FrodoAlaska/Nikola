...
void camera_create(Camera* cam, const f32 aspect_ratio, const Vec3& pos, const Vec3& target, const CameraMoveFn& move_fn) {
  ...
  cam->direction.x = nikol::cos((cam->yaw * NIKOL_DEG2RAD) * (cam->pitch * NIKOL_DEG2RAD));
  cam->direction.y = nikol::sin((cam->pitch * NIKOL_DEG2RAD));
  cam->direction.z = nikol::sin((cam->yaw * NIKOL_DEG2RAD)) * nikol::cos(cam->pitch * NIKOL_DEG2RAD);
...
void camera_update(Camera& cam) {
  ...
  cam.projection      = mat4_perspective((cam.zoom * NIKOL_DEG2RAD), cam.aspect_ratio, cam.near, cam.far);
